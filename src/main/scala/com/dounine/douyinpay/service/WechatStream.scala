            ...
            if (result.getOrElse(0) >= 18) {
              xmlResponse(
                Map(
                  ...
                  "CreateTime" -> message.createTime,
                  "MsgType" -> "text",
                  "Content" -> s"""抖音充值：<a href="https://open.weixin.qq.com/connect/oauth2/authorize?appid=${message.appid}&redirect_uri=https%3A%2F%2Fdouyin.61week.com%3Fccode%3D${message.eventKey}%26appid%3D${message.appid}%26platform%3Ddouyin%26bu%3Dhttps%3A%2F%2Fbackup.61week.com%2Fapi&response_type=code&scope=snsapi_base&state=${message.appid}&connect_redirect=1#wechat_redirect">充值链接</a>、无法访问建议切换网络重试""".stripMargin
                )
              )
            } else {
              xmlResponse(
